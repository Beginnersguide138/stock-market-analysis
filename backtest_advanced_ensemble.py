...
# アンサンブルモデル学習用ヘルパー関数
# ==========================================
def train_ensemble_model(t, i, df_train_base, features, seeds):
    ...
    if target_col in df_train_base.columns:
        ...
        if len(train_subset) >= 100:
            ...
            target_models = []
            for seed in seeds:
                # XGBoostのみ、シードを変えて複数作成
                xgb_model = xgb.XGBRegressor(n_estimators=100, max_depth=4, learning_rate=0.05, random_state=seed, tree_method='hist', n_jobs=1)
                xgb_model.fit(X_train, y_train)
                target_models.append(xgb_model)
            
            return f'{t}_{i}d', target_models
    return None, None

# ==========================================
# 6. 1ヶ月ローリング・バックテスト (毎週末に翌週5日間を予測)
# ==========================================
print("\nRunning Direct Multi-step rolling backtest (XGBoost Seed Averaging)...")
...
# XGBoostのアンサンブル用シード（3つの異なるモデルを作る）
seeds = [42, 123, 777] 

for base_date in tqdm(evaluation_dates):
    df_train_base = df_all[df_all['Date'] <= base_date].copy()
    
    tasks = [(t, i) for t in targets for i in range(1, 6)]
    results = Parallel(n_jobs=-1)(delayed(train_ensemble_model)(t, i, df_train_base, features, seeds) for t, i in tasks)
    
    models_ensemble = {k: v for k, v in results if k is not None}
    ...
    for i, target_date in enumerate(next_days):
        day_idx = i + 1
        target_key_open = f'Open_{day_idx}d'
        target_key_high = f'High_{day_idx}d'
        target_key_low = f'Low_{day_idx}d'
        target_key_close = f'Close_{day_idx}d'
        
        if target_key_close not in models_ensemble:
            continue
            
        def get_ensemble_pred(target_key):
            preds = []
            for model in models_ensemble[target_key]:
                preds.append(model.predict(X_base)[0])
            # 予測値の「平均(Mean)」を採用して安定化
            return np.mean(preds)
            
        pred_open_return = get_ensemble_pred(target_key_open)
        pred_high_return = get_ensemble_pred(target_key_high)
        pred_low_return = get_ensemble_pred(target_key_low)
        pred_close_return = get_ensemble_pred(target_key_close)
...
if not df_preds.empty:
    ...
    fig.add_trace(go.Candlestick(
        ...
        name='Predicted (XGBoost Seed Ensemble)',
...
fig.update_layout(
    title=f'【XGBoost Seed Ensemble】 {ticker} 1ヶ月ローリング・バックテスト',
    yaxis_title='株価 (円)',
...
output_html = f"backtest_seed_ensemble_{ticker_clean}_{timestamp}.html"
