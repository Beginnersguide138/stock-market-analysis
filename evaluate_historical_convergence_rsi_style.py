...
ticker = sys.argv[1] if len(sys.argv) > 1 else '1407.T'

END_FETCH_DATE = datetime.now().strftime('%Y-%m-%d')
START_FETCH_DATE = (datetime.now() - timedelta(days=365*2)).strftime('%Y-%m-%d')
TEST_END_DATE = END_FETCH_DATE
...
for tgt in unique_targets:
    ...
    if len(preds_for_tgt) >= 3:
        ...
        # New RSI-like normalization (0 to 100)
        # We want it to be highly sensitive:
        # Spread 0% -> Score 100
        # Spread 2% -> Score ~70 (Borderline stable)
        # Spread 4% -> Score ~30 (Panic zone)
        # Spread >=6% -> Score 0
        
        # Exponential decay function works well for this: e^(-k * spread)
        # We tune k so that spread=4.0 gives ~30. 
        # exp(-0.3 * 4.0) = exp(-1.2) = 0.301
        
        rsi_score = max(0, min(100, np.exp(-0.3 * volatility_spread) * 100))
...
fig = make_subplots(
    ...
    row_heights=[0.7, 0.3],
    subplot_titles=(f"Recent 1-Year Convergence ({ticker})", "Prediction Confidence Index (PCI: 0-100, <30 is Panic/Rebound Zone)")
)
...
if not df_conv.empty:
    ...
    fig.add_trace(go.Scatter(
        x=df_conv_plot['Target_Date'], y=df_conv_plot['RSI_Score'],
        mode='lines', name='PCI (Prediction Confidence Index)', line=dict(color='purple', width=2)
    ), row=2, col=1)
    
    # Add horizontal threshold lines like RSI
    fig.add_hline(y=70, line_dash="dash", line_color="green", row=2, col=1, annotation_text="70: High Conviction")
    fig.add_hline(y=30, line_dash="dash", line_color="red", row=2, col=1, annotation_text="30: Panic / Divergence")
    
    # Highlight areas below 30
...
fig.update_layout(
    ...
    showlegend=False,
    hovermode="x unified",
    yaxis2=dict(range=[0, 100]) # Force RSI scale
)
from datetime import datetime
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
filename = f"historical_pci_recent_1y_{ticker.replace('.T', '')}_{timestamp}.html"
fig.write_html(filename)
print(f"\nGenerated RSI-style PCI report: {filename}")
