...
# 最新日(3/2)などは予測ターゲット(未来)がないためTargetがNaNになる
# 学習用データ(df_train)はdropnaするが、推論の起点となる全データ(df_all)はターゲットのNaNを許容する
...
X_train_full = df.dropna().reset_index(drop=True)
...
# 7. 全データ(今日 3/2 までのデータを含む)を基準に、明日(3/3)以降の四本値を予測
test_date_start = pd.to_datetime('2026-03-03')
df_base = df_all[df_all['Date'] < test_date_start]
