...
for target_name, target_series in [('USD_JPY', y_fx), ('Sansan_Price', y_stock)]:
    mae_scores = []
    rmse_scores = []
    for fold, (train_idx, val_idx) in enumerate(tscv.split(X), 1):
        X_t, X_v = X.iloc[train_idx], X.iloc[val_idx]
        y_t, y_v = target_series.iloc[train_idx], target_series.iloc[val_idx]
        model_cv = xgb.XGBRegressor(n_estimators=100, max_depth=3, learning_rate=0.1, random_state=42)
        ...
        print(f"  {target_name} Fold {fold}: MAE={mae_scores[-1]:.4f}, RMSE={rmse_scores[-1]:.4f}")
    print(f"  {target_name} Avg MAE: {np.mean(mae_scores):.4f}, Avg RMSE: {np.mean(rmse_scores):.4f}")
